...
/*   Updated: 2025/03/11 20:39:15 by ljylhank         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > FOPEN_MAX - 1 || BUFFER_SIZE <= 0)
		return (NULL);
	if (leftovers_has_nl(leftovers[fd], &buffer))
		return (buffer);
	if (!read_file_until_nl(fd, &buffer))
	{
		fill_with_zeroes(leftovers[fd], BUFFER_SIZE + 1);
		return (NULL);
	}
	if (!getline_and_store_leftovers(leftovers[fd], &buffer))
	{
		free(buffer);
		return (NULL);
	}
